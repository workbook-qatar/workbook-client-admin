...
while ((match = regex.exec(c)) !== null) {
  ...
  const isSelfClosing =
    match[3] === "/" ||
    ["input", "img", "br", "hr"].includes(tag.toLowerCase());
  ...
  if (!isClosing) {
    stack.push({
      tag,
      line: c.substring(0, match.index).split(/\r\n|\n/).length,
      pos: match.index,
    });
  } else {
    // pop
    if (stack.length === 0) {
      console.error(
        `Error: Unmatched closing tag </${tag}> at line ${c.substring(0, match.index).split(/\r\n|\n/).length}`
      );
      break;
    }
    ...
    if (last.tag !== tag) {
      ...
      console.error(
        `Error: Mis-matched closing tag </${tag}> at line ${c.substring(0, match.index).split(/\r\n|\n/).length}. Expected </${last.tag}> (from line ${last.line}).`
      );
      break;
    }
  }
