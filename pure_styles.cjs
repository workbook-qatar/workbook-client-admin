...
content = content.replace(
  /<Label className="text-xs font-semibold text-gray-500">/g,
...
content = content.replace(
  /className=\"w-full text-left font-normal bg-white h-10 border-gray-200 text-\[13px\]\"/g,
  'className={cn("w-full text-left font-normal justify-between", STYLES.input)}'
);
content = content.replace(
  /className=\"w-full flex justify-between items-center text-left font-normal bg-white h-10 border-gray-200 text-\[13px\]\"/g,
  'className={cn("w-full flex justify-between items-center text-left font-normal", STYLES.input)}'
);
content = content.replace(
...
// 3. Cards
content = content.replace(
  /className=\"bg-white border border-gray-100 rounded-2xl shadow-\[0_2px_10px_rgb\(0,0,0,0\.02\)\] p-6 space-y-6 relative overflow-hidden transition-all duration-300\"/g,
  'className={STYLES.card + " space-y-6 relative overflow-hidden transition-all duration-300"}'
);

content = content.replace(
  /className=\"bg-white border text-center p-8 rounded-2xl shadow-\[0_2px_10px_rgb\(0,0,0,0\.02\)\] hover:shadow-md transition-shadow relative overflow-hidden group\"/g,
  'className={STYLES.card + " text-center relative overflow-hidden group hover:border-blue-300"}'
);

content = content.replace(
  /className=\"bg-white border border-gray-100 rounded-2xl shadow-\[0_2px_10px_rgb\(0,0,0,0\.02\)\] p-6 space-y-6 relative overflow-hidden\"/g,
  'className={STYLES.card + " space-y-6 relative overflow-hidden"}'
);
...
content = content.replace(
  /<div className="flex items-center gap-3 pb-[0-9] border-b border-gray-100">\s*<div[^>]*>[\s\S]*?<\/div>\s*<div>\s*<h3 className="text-lg font-bold text-gray-900">(.*?)<\/h3>[\s\S]*?<\/div>\s*<\/div>/g,
  "<h3 className={STYLES.sectionTitle}>$1</h3>"
);
...
content = content.replace(
  /className=\"h-11 px-8 bg-blue-600 hover:bg-blue-700 shadow-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed\"/g,
  `className="${primaryBtnTarget} h-[38px] flex items-center justify-center"`
);
content = content.replace(
  ...
  `className="${primaryBtnTarget} w-full h-[38px] mt-8 flex items-center justify-center"`
);
content = content.replace(
  /className=\"flex-\[2\] h-12 bg-green-600 hover:bg-green-700 shadow-sm hover:shadow text-white font-semibold tracking-wide transition-all\"/g,
...
content = content.replace(
  /className=\"flex-1 h-12 border-gray-200 text-gray-700 hover:bg-gray-50 rounded-xl font-semibold transition-colors\"/g,
  `className="flex-1 h-[38px] border border-gray-200 text-gray-700 hover:bg-gray-50 rounded-lg font-medium transition-colors"`
);
