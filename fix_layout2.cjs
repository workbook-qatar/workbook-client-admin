...
content = content.replace(
  /{currentStep === 0 && \(/g,
  '<div className="space-y-8 animate-in fade-in" id="step0">'
);
content = content.replace(
  /{currentStep === 1 && \(/g,
  '<div className="space-y-8 animate-in fade-in pt-6 border-t border-gray-200" id="step1">'
);
content = content.replace(
  /{currentStep === 2 && \(/g,
  '<div className="space-y-8 animate-in fade-in pt-6 border-t border-gray-200" id="step2">'
);
content = content.replace(
  /{currentStep === 3 && \(/g,
  '<div className="space-y-8 animate-in fade-in pt-6 border-t border-gray-200" id="step3">'
);
content = content.replace(
  /{currentStep === 99 && \(/g,
  '<div className="space-y-8 animate-in fade-in pt-6 border-t border-gray-200" id="step4">'
);
...
content = content.replace(
  /<div className="flex items-center gap-3 pb-3 border-b border-gray-100">\s*<div[^>]*>[\s\S]*?<\/div>\s*<div>\s*<h3 className="text-lg font-bold text-gray-900">(.*?)<\/h3>[\s\S]*?<\/div>\s*<\/div>/g,
  "<h3 className={STYLES.sectionTitle}>$1</h3>"
);
content = content.replace(
  /<div className="flex items-center gap-3 pb-2 border-b border-gray-100">\s*<div[^>]*>[\s\S]*?<\/div>\s*<div>\s*<h3 className="text-lg font-bold text-gray-900">(.*?)<\/h3>[\s\S]*?<\/div>\s*<\/div>/g,
  "<h3 className={STYLES.sectionTitle}>$1</h3>"
);
content = content.replace(
  /<div className="flex items-center gap-3 pb-4 border-b border-gray-100">\s*<div[^>]*>[\s\S]*?<\/div>\s*<div>\s*<h3 className="text-lg font-bold text-gray-900">(.*?)<\/h3>[\s\S]*?<\/div>\s*<\/div>/g,
  "<h3 className={STYLES.sectionTitle}>$1</h3>"
);

// Handle Summary "Text center py-4 bg-gradient..."
content = content.replace(
  /<div className="text-center py-4 bg-gradient[\s\S]*?<\/div>/,
  '<h3 className={STYLES.sectionTitle + " text-center"}>Review & Activate Profile</h3>'
);
