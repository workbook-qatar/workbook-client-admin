...
export function OrganizationSwitcher() {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      <SheetContent
        side="right"
        className="w-[400px] sm:w-[450px] p-0 flex flex-col bg-background"
      >
                  ...
                  >
                    <Avatar className="h-10 w-10 border border-border/50 bg-white shadow-sm rounded-lg shrink-0">
                      {org.logoUrl ? (
                        ...
                        <AvatarFallback
                          className={`${isActive ? "text-primary bg-primary/10" : "text-muted-foreground bg-muted/30"}`}
                        >
                    ...
                    <div className="flex-1 min-w-0 flex flex-col justify-center pr-2">
                      <h4
                        className={`text-sm truncate w-full ${isActive ? "font-bold text-foreground" : "font-medium text-foreground/80"}`}
                      >
                        {org.name}
                      </h4>
