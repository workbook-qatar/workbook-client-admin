...
export function MapView({
  ...
  const project = (lat: number, lng: number) => {
    ...
    const y =
      100 - ((lat - bounds.minLat) / (bounds.maxLat - bounds.minLat)) * 100; // Invert Y for SVG
    return { x, y };
  };
  ...
  return (
    <div
      className={cn(
        "relative w-full h-[600px] bg-slate-900 overflow-hidden rounded-xl shadow-inner border border-white/10",
        className
      )}
    >
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none">
        ...
        {trips.map(trip => {
          ...
          const isActive =
            trip.status === "active" || trip.currentStatus === "in-progress";

          let strokeColor = isUnassigned
            ? "#f97316"
            : isActive
              ? "#22c55e"
              : "#3b82f6";
      ...
      <div className="absolute bottom-4 right-4 flex flex-col gap-2">
        <button className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 text-white flex items-center justify-center hover:bg-slate-700 transition">
          +
        </button>
        <button className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 text-white flex items-center justify-center hover:bg-slate-700 transition">
          -
