          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold tracking-tight flex items-center gap-2">
                ...
                <Badge
                  variant="secondary"
                  className="bg-amber-100 text-amber-700 hover:bg-amber-100 border-amber-200 gap-1.5 font-normal"
                >
              ...
              <p className="text-sm text-gray-500">
                Browse and add standard industry packages to your catalog
              </p>
            </div>
          </div>
          <div>
                ...
                {PRESET_CATEGORIES.map(cat => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <span
                        className={`font-medium ${selectedCategory === cat.id ? "text-blue-900" : ""}`}
                      >
                        {cat.name}
                      </span>
                    </div>
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                    {activeCategory.packages.map(pkg => {
                      ...
                      return (
                        <Card
                          key={pkg.id}
                          className="p-5 border-gray-200 hover:border-blue-200 transition-all hover:shadow-md group relative overflow-hidden"
                        >
                          {isAdded && (
                          ...
                          <div className="flex justify-between items-start gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-1">
                                <Badge
                                  variant="outline"
                                  className="font-mono text-[10px] text-gray-400 border-gray-200"
                                >
                                  {pkg.code}
                                </Badge>
                                {pkg.materials && (
                                  <Badge
                                    variant="secondary"
                                    className="text-[10px] h-5 bg-purple-50 text-purple-700 hover:bg-purple-50 border-0"
                                  >
                                    + Materials
                                  </Badge>
                                )}
                            ...
                            <div className="flex-none pt-2">
                              {isAdded ? (
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    handleAddPackage(pkg, activeCategory)
                                  }
                                >
                                  Add to Services
                                </Button>
