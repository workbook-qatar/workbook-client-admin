...
const STYLES = {
  sectionTitle:
    "text-[16px] font-semibold text-gray-900 mb-5 relative pl-3 before:absolute before:left-0 before:top-1 before:w-[3px] before:h-4 before:bg-blue-600 before:rounded-full",
  label: "text-[13px] text-gray-600 font-medium mb-1.5 block",
  input:
    "h-[38px] text-sm bg-gray-50/50 border-gray-200 focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 rounded-lg transition-all",
...
export default function CreatePackage() {
  ...
  useEffect(() => {
    ...
    if (storedSkills) {
      ...
      setAvailableSkills(
        skillsObj.map((s: any) => ({ value: s.id, label: s.name }))
      );
    } else {
    ...
    if (isEditMode && packageId) {
      ...
      const uniquePackages = Array.from(
        new Map(allPackages.map((item: any) => [item.id, item])).values()
      );
      ...
      if (pkg) {
        ...
        setDuration(
          String(pkg.duration).replace(" hours", "").replace(" hour", "")
        ); // strip text if stored
  ...
  const isValid =
    ...
    selectedAreas.every(id => areaPrices[id] && parseFloat(areaPrices[id]) > 0);
        ...
        <div className="flex-none px-8 py-5 border-b border-gray-200 bg-white/80 backdrop-blur-md sticky top-0 z-20">
          <div className="flex items-center justify-between max-w-5xl mx-auto w-full">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button
                className="bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-600/20 gap-2 rounded-lg px-6"
                disabled={!isValid}
                onClick={handleSave}
              >
          ...
          <div className="max-w-3xl mx-auto space-y-8 pb-32">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="e.g. Deep Clean - 2 Bedroom"
                      ...
                      <SelectContent>
                        ...
                        <div className="p-1 border-t mt-1">
                          <Button
                            ...
                            onClick={() =>
                              setLocation("/services/category/create")
                            }
                          >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      value={staffReq}
                      onChange={e => setStaffReq(parseInt(e.target.value))}
                      min={1}
                      className={STYLES.input}
                    />
                  </div>
                  ...
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                        role="combobox"
                        className={`w-full justify-between font-normal ${STYLES.input} h-auto min-h-[38px] px-3 py-2 text-left`}
                      >
                        {selectedSkills.length > 0 ? (
                          <div className="flex flex-wrap gap-1.5">
                            {selectedSkills.map(val => (
                              ...
                              >
                                {
                                  availableSkills.find(s => s.value === val)
                                    ?.label
                                }
                                <span
            ...
            <Card className={STYLES.card}>
              <div className="flex items-center justify-between mb-5">
                <h3 className={STYLES.sectionTitle.replace("mb-5", "mb-0")}>
                ...
                <Button
                  ...
                  onClick={() =>
                    window.open("/settings/service-areas/create", "_blank")
                  }
                >
                  + Add Service Area
                </Button>
              ...
              <div className="space-y-6">
                <div>
                  <Label className={STYLES.label}>
                    Select Service Areas <span className="text-red-500">*</span>
                  </Label>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-gray-200 bg-gray-50 text-left">
                          <th className="py-2.5 px-4 font-medium text-gray-500 w-[50%]">
                            Service Area
                          </th>
                          <th className="py-2.5 px-4 font-medium text-gray-500">
                            Price (QAR)
                          </th>
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {selectedAreas.map(areaId => {
                          const area = MOCK_SERVICE_AREAS.find(
                            a => a.id === areaId
                          );
                          ...
                          return (
                            <tr
                              key={areaId}
                              className="group bg-white hover:bg-blue-50/30 transition-colors"
                            >
                              ...
                              <td className="py-3 px-4">
                                <div className="flex items-center gap-2">
                                  <div className="relative w-full max-w-[140px]">
                                    <span className="absolute left-3 top-2 text-xs text-gray-400 group-focus-within:text-blue-500">
                                      QAR
                                    </span>
                                    <Input
                                      type="number"
                                      placeholder="0.00"
                                      className="h-8 pl-10 text-right font-medium"
                                      value={price}
                                      onChange={e =>
                                        updatePrice(areaId, e.target.value)
                                      }
                                    />
                                  </div>
                                  <div className="w-6">
                                    {price && (
                                      <button
                                        title="Apply all"
                                        onClick={() => applyPriceToAll(areaId)}
                ...
                {selectedAreas.length === 0 && (
                  <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
                    ...
                    <p className="text-sm text-gray-400">
                      Select service areas to configure pricing
                    </p>
                  </div>
                )}
              </div>
            ...
            <Card className={STYLES.card}>
              ...
              <div>
                ...
                <Textarea
                  ...
                  className="min-h-[100px] text-sm resize-none bg-gray-50/50 border-gray-200 rounded-lg"
                />
              </div>
            </Card>
          </div>
