...
export default function ServiceAreasList() {
  ...
  const filteredAreas = MOCK_SERVICE_AREAS.filter(
    area =>
        ...
        <div className="px-8 py-6 border-b bg-white flex items-center justify-between sticky top-0 z-10">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Define geographical zones where you provide services.
            </p>
          </div>
          <Button
            onClick={() => setLocation("/settings/service-areas/create")}
        ...
        <div className="p-8 max-w-5xl mx-auto w-full">
          ...
          <div className="grid gap-4">
            {filteredAreas.map(area => (
              <Card
                key={area.id}
                className="p-5 flex items-center justify-between hover:shadow-md transition-all cursor-pointer group bg-white border-gray-100"
                onClick={() => {}}
              >
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h3 className="font-semibold text-gray-900 text-lg flex items-center gap-2">
                      {area.name}
                      <span className="text-xs font-normal text-gray-400 border px-1.5 py-0.5 rounded bg-gray-50">
                        {area.region}
                      </span>
                    </h3>
            ...
            {filteredAreas.length === 0 && (
              <div className="text-center py-16 bg-white rounded-xl border border-dashed border-gray-200">
                ...
                <p className="text-gray-500 mt-1">
                  No service areas match your search for "{searchQuery}"
                </p>
              </div>
            )}
          </div>
