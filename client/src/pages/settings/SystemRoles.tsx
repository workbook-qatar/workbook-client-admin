...
const DEFAULT_ROLES: SystemRole[] = [
  {
    ...
    name: "Admin",
    description:
      "Unrestricted access to all modules and system configurations.",
  ...
  },
  {
    ...
    name: "Manager",
    description:
      "Full access to operational modules, limited access to financial settings.",
    isSystemDefault: true,
    permissions: [
...
export default function SystemRoles() {
  ...
  const openEdit = (role: SystemRole) => {
    if (role.isSystemDefault) {
      toast.info(
        "System default roles cannot be fully edited. Create a custom role instead."
      );
      return;
    }
        ...
        <div className="px-8 py-6 border-b bg-white flex items-center justify-between sticky top-0 z-10 shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[13px] text-gray-500 mt-1">
                Manage functional roles, module permissions, and dashboard
                access.
              </p>
            </div>
          </div>
          ...
          >
            <DialogTrigger asChild>
              <Button
                onClick={resetForm}
                className="bg-purple-600 hover:bg-purple-700 text-white gap-2 shadow-md shadow-purple-600/20 font-semibold px-5 rounded-xl transition-all"
              >
            ...
            <DialogContent className="max-w-2xl">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <Label className="text-[13px] font-bold text-gray-900 flex items-center gap-2">
                      <ShieldCheck className="h-4 w-4 text-purple-600" /> Module
                      Permissions
                    </Label>
                    <Button
                      variant="outline"
                  ...
                  <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3 max-h-[300px] overflow-y-auto">
                    {formData.permissions.length === 0 && (
                      <div className="text-center py-6 text-sm text-gray-500 italic">
                        No permissions added. Click "Add Permission" above.
                      </div>
                    )}
                    {formData.permissions.map((perm, idx) => (
                      <div
                        key={idx}
                        className="flex flex-col md:flex-row items-center gap-3 bg-white p-2.5 rounded-lg border border-gray-200 shadow-sm animate-in fade-in zoom-in-95"
                      >
              ...
              <DialogFooter className="pt-4 border-t border-gray-100">
                ...
                <Button
                  onClick={handleSave}
                  className="bg-purple-600 hover:bg-purple-700 rounded-xl font-bold shadow-sm px-6"
                >
                  Save Role
                </Button>
              </DialogFooter>
        ...
        <div className="p-8 max-w-6xl mx-auto w-full space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredRoles.map(role => (
              <Card
                key={role.id}
                className="bg-white border-gray-200/60 shadow-sm hover:shadow hover:border-purple-200 transition-all group overflow-hidden flex flex-col"
              >
                <div className="p-6 pb-5 flex flex-col flex-1 border-b border-gray-50/50 relative">
                  {role.isSystemDefault && (
                    <div className="absolute top-6 right-6">
                      <Badge
                        variant="secondary"
                        className="bg-gray-100 text-gray-600 font-semibold border-gray-200 uppercase tracking-widest text-[9px] px-2 py-0.5 shadow-sm"
                      >
                        System Default
                      </Badge>
                    </div>
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    <div
                      className={`h-12 w-12 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${role.name === "Admin" ? "bg-red-50 text-red-600" : role.isSystemDefault ? "bg-blue-50 text-blue-600" : "bg-purple-50 text-purple-600"}`}
                    >
                      {role.name === "Admin" ? (
                  ...
                  <div className="bg-gray-50/80 rounded-xl p-4 border border-gray-100 flex-1 space-y-2.5">
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-4">
                      {role.permissions.slice(0, 4).map((p, i) => (
                        <div
                          key={i}
                          className="flex items-start gap-1.5 align-top"
                        >
                          <Check className="h-3.5 w-3.5 text-green-500 mt-0.5 shrink-0" />
                          <div>
                            <div className="text-[11px] font-bold text-gray-900 whitespace-nowrap overflow-hidden text-ellipsis max-w-[120px]">
                              {p.module}
                            </div>
                  ...
                  {!role.isSystemDefault ? (
                    <div className="flex flex-row items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 text-xs font-semibold text-gray-600 hover:text-purple-700 hover:bg-purple-50 rounded-lg px-3"
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 text-gray-400 hover:text-red-700 hover:bg-red-50 rounded-lg"
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      className="h-8 text-xs font-semibold text-gray-400 cursor-not-allowed"
                    >
                      Protected
                    </Button>
                  )}
            ...
            {filteredRoles.length === 0 && (
              <div className="text-center py-16 bg-white rounded-2xl border border-dashed border-gray-200 col-span-1 md:col-span-2 shadow-sm">
                ...
                <p className="text-gray-500 text-sm mb-4">
                  You haven't created any matching roles yet.
                </p>
                <Button
                  variant="outline"
                  onClick={resetForm}
