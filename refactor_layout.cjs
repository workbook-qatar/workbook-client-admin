...
content = content.replace(
  /{currentStep === 1 && \(/g,
  '<div className="space-y-8 animate-in fade-in pt-6 border-t border-gray-200">'
);
content = content.replace(
  /{currentStep === 2 && \(/g,
  '<div className="space-y-8 animate-in fade-in pt-6 border-t border-gray-200">'
...
content = content.replace(
  /<div className="pt-8 border-t border-gray-100 flex items-center justify-between">[\s\S]*?<\/div>/g,
  ""
);
content = content.replace(
  /<div className="pt-4 flex items-center justify-between">[\s\S]*?<\/div>/g,
  ""
);
...
content = content.replace(
  ...
  '<Card className={STYLES.card + " text-center relative overflow-hidden group border-blue-100"}>'
);
...
content = content.replace(
  ...
  '<Card className={STYLES.card + " space-y-6 relative overflow-hidden transition-all duration-300"}>'
);
...
content = content.replace(
  ...
  '<Card className={STYLES.card + " space-y-6 relative overflow-hidden"}>'
);
...
content = content.replace(
  /<div className="flex items-center gap-3 pb-[0-9] border-b border-gray-100">\s*<div.*?<\/div>\s*<div>\s*<h3.*\n.*\n\s*<\/div>\s*<\/div>/g,
  match => {
    // Extract title text
...
content = content.replace(
  /<h3 className="text-lg font-bold text-gray-900">/g,
